...
bool DeviceCollect::ModbusTcpDevicesMapping(Device &device)
{
    ...
    modbusTcpDevice.id = device.id;
    modbusTcpDevice.ip = device.ip;
    modbusTcpDevice.port = device.port;
    for(int i = 0; i < device.CollectionFields.size(); i++){
